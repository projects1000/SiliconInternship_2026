...
export class AttendanceTableComponent implements OnInit {
  @Input() selectedGroup = 'Group A';
  @Output() attendanceSaved = new EventEmitter<any>();
  ...
  ngOnInit() {
    ...
    if (savedData) {
      this.groupStudents = JSON.parse(savedData);
    }
  }

  saveToLocalStorage() {
    localStorage.setItem('attendanceData', JSON.stringify(this.groupStudents));
  }
  ...
  getPresentCount() {
    return this.students.filter((student: any) => student.present).length;
  }
  ...
  deleteStudent(student: any) {
    const currentGroup = this.groupStudents[this.selectedGroup];
    const index = currentGroup.indexOf(student);
  ...
  addStudentToGroup(group: string, studentName: string) {
    ...
    const newRoll = students.length > 0 ? students[students.length - 1].roll + 1 : 100;
    students.push({
      roll: newRoll,
      name: studentName,
      present: false
