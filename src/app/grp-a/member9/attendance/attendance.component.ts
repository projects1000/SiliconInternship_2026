...
export class AttendanceComponent implements OnInit {
  ...
  /**
   * ── SECURITY RULE PATCH ──────────────────────────────────────────────────
   * Enforces that updates are only permitted for the current day or past logs,
   * completely freezing interactions with upcoming/future date parameters.
   */
  isDateEditable(dateStr: string): boolean {
    if (this.isLocked) return false;

    // Fetch the real, exact current calendar date tracking string
    const realTodayStr = this.formatDateKey(new Date());

    // Instantly terminate operations if the column target belongs to the future
    if (dateStr > realTodayStr) return false;
